...
Item {
  ...
  function syncPlayingOrder() {
    ...
    var preferred = preferredPlayerKey

    for (var i = 0; i < players.length; i++) {
      var p = players[i]
      var key = playerKey(p)
      if (!key || isProxyPlayer(p)) continue

      // Keep history while paused. Only a transition into playback may take
      // the keys; metadata updates and lingering audio streams cannot do so.
      playing[key] = !!p.isPlaying
      if (playerStartedAt[key] !== undefined) next[key] = playerStartedAt[key]
      if (p.isPlaying && !previouslyPlaying[key]) {
        serial += 1
        next[key] = serial
        preferred = key
      }
    ...
    // Retain the key even if that player closes. Resuming an unrelated paused
    // player is surprising; wait for playback or an explicit source selection.
  ...
  function selectActivePlayer() {
    return playerForKey(preferredPlayerKey)
  }
  ...
  function playerForAction(action, targetKey) {
    // Unsupported actions must not spill into another app (e.g. Next in a
    // browser video must not skip a Spotify track or start a paused podcast).
    return targetKey ? playerForKey(targetKey) : activePlayer
  }
  ...
  function statusJson() {
    ...
    return JSON.stringify({
      ...
      preferredPlayerKey: preferredPlayerKey,
